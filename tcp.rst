...
Swift Start
^^^^^^^^^^^
The goal of the scheme is to make the best possible use of packet pair and pacing 
to allow a TCP to rapidly open its window on a link with a large bandwidthxdelay 
product, while fundementally keeping TCP working much as we know it today.

Initial Congestion Window = 4

We estimate the bandwidth by calculating the differnece in time of recieving the
first and second acknowledgement. This difference is then divided by the size of
the segment.

.. math::  estBW = (m_secondAcked - m_firstAcked)/tcb->m_segmentSize;

We estimate the capacity of the link by calculating the product of estimated 
bandwidth and the time difference between sending the first packet and recieving 
its acknowledgement.

.. math::  estCap = estBW * rtt.GetSeconds();

The new congestion window is given by taking a fraction gamma of the estimated 
capacity, gamma varies between 1 and 1/8th.

.. math::  estCap / m_gamma;


More informations at: www.icir.org/mallman/pubs/PRAKS02/PRAKS02.ps
