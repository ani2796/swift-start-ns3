...
{
  std::string transport_prot = "TcpSwiftStart";
  double error_p = 0.0;
  ...
  CommandLine cmd;
  cmd.AddValue ("transport_prot", "Transport protocol to use: TcpNewReno, TcpSwiftStart, "
  ...
  else if (transport_prot.compare ("TcpSwiftStart") == 0)
    {
      Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpSwiftStart::GetTypeId ()));
    }
  ...
  for (uint16_t i = 0; i < sources.GetN (); i++)
    {
      ...
      if (transport_prot.compare ("TcpNewReno") == 0
          ...
          || transport_prot.compare ("TcpSwiftStart") == 0)
        {
